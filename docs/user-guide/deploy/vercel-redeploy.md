...
当我们修改 NotionConfig 的配置或文章内容后，Notion Repo 站点会自动同步 Notion 的数据。
...
由于Notion Repo 站点的每个页面都有独立的缓存，当您修改站点的菜单时，旧版本的菜单可能会被缓存，导致菜单栏无法与所有页面一起更新。
...
利用这个特性，如果我们要让vercel重新部署，一种做法就是任意修改一下Github上的Notion Repo代码，即可触发redeploy。
