...
export async function fetchPageFromNotion(pageId) {
  ...
  const result = {
    ...
    fans_code: (() => {
      ...
      let codes = []
      if (Array.isArray(raw)) {
        codes = raw.map(item => (Array.isArray(item) ? item[0] : (typeof item === 'object' ? item?.name : String(item)))).filter(Boolean)
      } else if (raw) {
        codes = String(raw).split(/[,，;\s/]+/).filter(Boolean)
      }
      // 核心对齐：仅当是粉丝文章时，根据页面底层 UUID 自动对齐 Notion 模板公式生成的大写 6 位独立随机专属码
      const cleanId = String(pageId || '').replace(/-/g, '').toLowerCase()
      if (cleanId.length >= 32) {
        const formulaCode = cleanId.substring(26, 32).toUpperCase()
        if (formulaCode && !codes.some(c => String(c).toUpperCase() === formulaCode)) {
          codes.unshift(formulaCode)
        }
      }
      return codes.join(',')
    })(),
