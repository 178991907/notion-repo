...
export function adjustPageProperties(properties, NOTION_CONFIG) {
  ...
  // 粉丝专区标识与独立验证码：严格以是否勾选 fans (checkbox/select/文本) 为准
  const rawFans = properties.fans
  const isFans = Boolean(
    rawFans === true ||
    rawFans === 'true' ||
    rawFans === 'Yes' ||
  ...
  if (isFans) {
    ...
    // 核心对齐：仅当文章确实开启了 fans 时，才根据页面底层 UUID 自动对齐 Notion 模板公式生成的大写 6 位独立随机专属码
    // Notion 公式规则：prop("fans") ? upper(substring(replace(id(), "-", ""), 26, 32)) : ""
    const cleanId = String(properties?.id || '').replace(/-/g, '').toLowerCase()
    if (cleanId.length >= 32) {
      const formulaCode = cleanId.substring(26, 32).toUpperCase()
      if (formulaCode && !fansCodes.some(c => String(c).toUpperCase() === formulaCode)) {
        fansCodes.unshift(formulaCode)
      }
    }
    ...
    properties.fans_code = ''
    properties.fans_codes = []
  }
}
...
function shouldUseMappedCategorySlug(properties, NOTION_CONFIG) {
  ...
  const categoryMap =
    NOTION_CONFIG?.POST_URL_PREFIX_MAPPING_CATEGORY ??
    siteConfig(
