...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  if (globalSnapshot) {
    ...
    if (!hasVal(val)) {
      // 仅读取 NOTION_CONFIG（来自 Notion 数据库或服务端注入的配置），不在此处读取 THEME_CONFIG 默认值以防阻塞后台覆盖
  ...
  // 3. 判断是否为主题专有配置（例如 HEO_* 等）。后台管理保存的配置绝对优先于主题硬编码默认值！
  ...
  if (isThemeKey) {
    // 主题专有配置：后台管理覆盖配置绝对最优先
    if (currentAdminOverrides && currentAdminOverrides[key] !== undefined) {
      val = currentAdminOverrides[key]
    }
    // 若后台未覆盖，且当前 val 为空，则依次从 extendConfig 和 THEME_CONFIG 默认值回退
    ...
    // 通用非主题配置：局部传参 extendConfig 优先，其次为后台 adminOverrides 覆盖
    if (!hasVal(val) && extendConfig && extendConfig[key] !== undefined) {
      val = extendConfig[key]
    }
