...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  switch (key) {
    ...
    case 'UUID_REDIRECT':
      ...
      return convertVal(
        getValue(extendConfig[key], getValue(adminOverrides?.[key], getValue(BLOG[key], defaultVal)))
      )
    default:
  }
  ...
  // 2. 其次 有传入的extendConfig (例如 SSG 时传入的 NOTION_CONFIG)，则尝试读取
  if (!hasVal(val) && extendConfig) {
    val = extendConfig[key]
  }

  // 3. 再次 从管理员打包配置 adminOverrides 中读取
  if (!hasVal(val) && adminOverrides && adminOverrides[key] !== undefined) {
    val = adminOverrides[key]
  }

  // 4. 再次 NOTION没有找到配置，则会读取blog.config.js文件
  if (!hasVal(val)) {
    val = BLOG[key]
  }
