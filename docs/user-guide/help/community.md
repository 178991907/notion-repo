# 3.社群讨论
> 迁移自：[3.社群讨论](https://docs.tangly1024.com/article/chat-community)
> 发布日期：2021-3-18
> 最后编辑：2025-8-20
> 原栏目：👨‍👦‍👦 获取帮助
> 标签：网站

::: tip 提示
这里的《交流社群》指的是《Notion站长交流》，本人从未开设“交易技术群”等其它投资交易相关的社群！
如有关投资交易的问题咨询，请通过邮或小助理的方式私下与我链接。
:::

::: warning 注意
加入站长社群前请先阅读[《帮助须知-社群守则》](/user-guide/help/community-rules)

个人精力有限，目前每天的邮件、Msg、留言看不完，因此我无法参与群讨论，但是群里有很多乐于分享的前辈可以交流。
:::


## 站长微信群

::: tip 提示
群里汇聚了各领域的大神们，感谢你们的经验分享！以下是部分群消息截图：

![Untitled](/legacy/3c3360edc775b344.jpg)
![Untitled](/legacy/9135b9a3e0c754ab.jpg)
![Untitled](/legacy/d1f7e22eae20ad60.jpg)
![Untitled](/legacy/3c6a7b2a6cfd686e.jpg)
:::

#### 入群方式(点击展开)
::: tip 提示
为维护良好的交流环境，微信群需要您支付**￥29.9**的入群费（请小助理喝一杯咖啡）。
:::

对Notion Repo代码做出[贡献](https://github.com/notionnext-org/Notion Repo/graphs/contributors)，或对《Notion Repo帮助手册》有特别贡献的成员，可以免费入群。
请添加小助理微信，并在申请原因中备注：`**付费入群-您的称呼**`** ， **助理会给您发一个收款码，支付成功后，助理会拉您进群❤
![image.png](/legacy/ada2c3f35445f279.png)
#### 助理微信 （点击展开）
::: tip 提示
小助理负责群维护与付费咨询的对接，小助理无法提供技术答疑。
:::

若有BUG或需求，请提交[Github](/user-guide/help/feedback#3c5237a1e3a4438ea6dcb519b543b985)，以便我能更全面了解你的问题，并在后续版本中跟踪任务进度。
::: tip 提示
微信加好友过频繁、拉人入群次数过频繁都可能被限制聊天功能，如果遇到小助理微信异常的情况，请通过[邮件联系](/user-guide/help/feedback#9e3419df5992499d965c278eb8b9fd02)我。
:::

      - **扫下方二维码(**或搜索账号`crush_auv`**)**添加小助理微信
![扫码添加好友](/legacy/0e9b5ac41f49feba.jpg)
::: tip 提示
感谢您对 Notion 站长交流群的关注和支持，期待您的加入，共同打造一个积极、有益的交流平台！
:::


## Telegram群

> **https://s3-us-west-2.amazonaws.com/secure.notion-static.com/f61bdcd6-773d-4c7f-85f9-d6a86e12ffce/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20230327101619.png** 主要面向海外国际友人，本人比较少使用telegram。
>
[Notion Repo交流群](https://t.me/+sOvnlf2QKZQ3NDU1)


## 开发者社群

- 我们只有**站长社群**，没有另外建立开发者社群，**开发者们通常在issue中进行留言沟通**。站长社群需要付费加入，但对于Notion Repo的开发者们可免费入群。

- 您可以通过以下方式成为开发者：
  - 您需要自行通过**issue讨论区**或其它方式，定位到需要改动的代码，并且提交一个**解决问题的PR**，PR成功通过合并后即可成为[Notion Repo的开发者](https://github.com/notionnext-org/Notion Repo/graphs/contributors)。

- 开发者们请联系小助理加群或与我对接。


## 其它

<details>
<summary>废弃频道</summary>

<details>
<summary>~~Discord聊天~~</summary>

[Join the Notion Repo Discord Server!](https://discord.gg/9KvvSy2TUB)

Check out the Notion Repo community on Discord - hang out with 1 other members and enjoy free voice and text chat.
::: tip 提示
精力有限，Discord频道停止维护
:::

</details>

</details>

## 原文链接

https://docs.tangly1024.com/article/chat-community
