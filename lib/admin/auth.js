...
function getSecret() {
  ...
  if (adminPass) return adminPass + '_notion_repo_secret'
  
  console.warn('【强烈警告】未设置 ADMIN_SECRET 和 ADMIN_PASSWORD，系统正在使用弱默认密钥派生机制，存在严重安全风险！请立即在环境变量中配置 ADMIN_SECRET。')
  
  // 基于当前站点唯一特征派生跨 Lambda 实例确定性安全密钥，防止 Serverless 多实例随机数不一致
  const deterministicSeed = process.env.NOTION_PAGE_ID || 'notion_repo_admin_stable_seed_2026'
  return crypto.createHash('sha256').update(deterministicSeed + '_admin_secret_salt_2026').digest('hex')
}
