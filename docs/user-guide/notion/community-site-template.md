...
Notion Repo 默认适合个人博客和独立页面。如果你想把同一个 Notion 数据库扩展成社区、团队、开源项目或活动官网，可以在原有文章字段上增量加入 `Member` 与 `Event` 类型。
...
这样可以在不破坏原博客流程的前提下，把 Notion Repo 从个人博客逐步扩展为社区或组织站点。
