...
export async function fetchPageFromNotion(pageId) {
  ...
  const result = {
    ...
    vip_level: (() => {
      const raw = postInfo?.properties?.vip_level?.[0]?.[0] || postInfo?.properties?.level?.[0]?.[0] || postInfo?.properties?.vip_level || postInfo?.properties?.level
      if (typeof raw === 'string' && /^(svip|高级会员|尊享会员)$/i.test(raw.trim())) {
        return 'SVIP'
      }
      return postInfo?.properties?.vip ? 'VIP' : null
    })(),
    fans: Boolean(
    ...
    fans_code: String(postInfo?.properties?.fans_code?.[0]?.[0] || postInfo?.properties?.fans_code || '').trim(),
    blockMap
  }
