...
export function adjustPageProperties(properties, NOTION_CONFIG) {
  ...
  // 会员等级标识：VIP (普通会员) 或 SVIP (高级会员)
  const rawVipLevel = properties.vip_level || properties.level
  if (typeof rawVipLevel === 'string' && /^(svip|高级会员|尊享会员)$/i.test(rawVipLevel.trim())) {
    properties.vip_level = 'SVIP'
    properties.vip = true
  } else if (properties.vip) {
  ...
  properties.fans_code = properties.fans_code ? String(properties.fans_code).trim() : ''
  if (properties.fans_code) {
    properties.fans = true // 填写了粉丝专属验证码自动归入粉丝专区
  }
}
