 ...
 * 若勾选 fans 但未填 fans_code，自动填充默认暗号
 * 若勾选 vip 但未填 vip_level，自动填充 VIP 等级
 * @param {string} defaultPasscode 默认粉丝验证码
 */
export async function syncNotionArticleProperties(defaultPasscode = '888888') {
  const client = getNotionClient()
  ...
  for (const page of response.results) {
    ...
    if (needFans || needVip) {
      const updateProps = {}
      if (needFans) {
        updateProps.fans_code = { multi_select: [{ name: String(defaultPasscode).trim() || '888888' }] }
        updatedFansCount++
      }
      if (needVip) {
