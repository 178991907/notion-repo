...
This document is for **core maintainers and frequent contributors**. Read it together with [Contributing](https://github.com/notionnext-org/Notion Repo/blob/main/CONTRIBUTING.md) and [Contribution workflow](./CONTRIBUTION_WORKFLOW.en.md). The goal is to welcome contributions while **reducing large, hard-to-control changes** that destabilize `main` and fork ecosystems.
