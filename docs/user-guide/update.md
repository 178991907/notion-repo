...
## 4.21.0 企业级安全加固升级提示 (重磅发布)

`4.21.0` 是全栈安全深度重构与工业级防御加固的大版本发布。针对开源代码库的安全性与多端协同，全盘修补了 26 项潜在漏洞，**他人 Fork 本项目或新站点部署 100% 顺畅，绝无报错**！

### 核心升级亮点与无感平滑迁移：

1. **会员密码自动升级 (Password Auto-Upgrade)**：
   - 彻底废除明文存储，密码存储全面迁移至 `bcrypt` 加盐哈希；
   - **完全无需手动重置密码**：老站点升级后，历史会员无需重置密码。下次正常登录时，系统在验证通过后会在后台**全自动、静默将密码升级为 bcrypt 格式并写回 Notion 数据库**，0 感知过渡！

2. **Gitalk OAuth 安全后端代理无缝兼容**：
   - 为防止 GitHub OAuth App 凭证暴露，系统新增了 `/api/proxy/gitalk-token` 后端代理端点；
   - Gitalk 前端组件已全自动适配该代理端点，原有使用流程不变，无需开发者手动重构代码。

3. **Edge 运行时真鉴权中间件**：
   - 后台路由 `/admin` 与所有管理接口 `/api/admin/*` 全面接入 Edge 兼容的 HMAC-SHA256 签名鉴权；
   - 拦截任何伪造 Token Cookie 的非法访问，保障管理后台绝对安全。

4. **15+ 套现代主题富文本评论 XSS 彻底免疫**：
   - 全站评论组件引入 `isomorphic-dompurify` 强制清洗，免疫第三方评论接口注入。

5. **环境变量升级建议 (选配)**：
   同步代码后，推荐在部署平台补充以下安全私钥（未配置时系统将优雅降级，不会导致构建或运行崩溃）：
   ```bash
   ADMIN_SECRET=你的后台JWT私钥
   MEMBER_AUTH_SECRET=你的会员JWT私钥
   CRON_SECRET=你的定时任务签名私钥
   FANS_CODE_SECRET=你的粉丝暗号签名私钥
   ```

普通站点在 GitHub 点击 **Sync fork** 同步最新 `main` 后，在 Vercel 点击 **Redeploy** 重新部署即可享受全新安全防护！
