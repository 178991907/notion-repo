...
export function getMembersDatabaseId() {
  return process.env.NOTION_MEMBERS_DATABASE_ID || global.__notionMembersDatabaseId || (process.env.NODE_ENV !== 'test' ? '3df01bb0-e99b-8112-bc28-c1876d5d0cfd' : null)
}
...
export function getInvitesDatabaseId() {
  return process.env.NOTION_INVITES_DATABASE_ID || global.__notionInvitesDatabaseId || (process.env.NODE_ENV !== 'test' ? '3df01bb0-e99b-8130-9ffe-e4c8350bb1eb' : null)
}
...
export async function initMemberDatabases() {
  ...
  if (!client) {
    throw new Error('未配置有效的 Notion API Token (NOTION_ACCESS_TOKEN)')
  }
  const rootPageId = (process.env.NOTION_PAGE_ID || '').replace(/-/g, '').trim()
  if (!rootPageId) {
    throw new Error('未配置 NOTION_PAGE_ID，无法定位父页面')
  }
  ...
  if (!invitesDbId || !membersDbId) {
    throw new Error('自动创建数据库受限。请检查：1. 是否已在 Notion 根页面右上角点击「··· ➔ Connect to」授权您的集成；2. 集成是否开启了 Insert Content 权限。')
  }
...
export async function createMember({ username, password, inviteCode = '', expireDate = null, remark = '', level = 'VIP' }) {
  ...
  if (!client || !databaseId) {
    throw new Error('系统尚未配置或探测到 Notion 会员数据库（NOTION_MEMBERS_DATABASE_ID）')
  }
...
export async function createInviteCode({ code, level = 'VIP', maxUses = 1, days = 0, remark = '' }) {
  ...
  const databaseId = await resolveInvitesDatabaseId(client)
  if (!client || !databaseId) {
    throw new Error('未配置或未探测到邀请码数据库（NOTION_INVITES_DATABASE_ID）')
  }
...
export async function updateInviteCodeStatus(pageId, newStatus = 'Disabled') {
  const client = getNotionClient()
  const databaseId = await resolveInvitesDatabaseId(client)
  if (!client || !databaseId) throw new Error('未配置或未探测到邀请码数据库')
