...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  const findPost = () => {
    ...
    const normalize = str => (typeof str === 'string' ? str : String(str || '')).replace(/^\/+/, '').trim()
    const targetSlug = normalize(fullSlug)
    const targetLast = normalize(lastSegment)

    console.log('[findPost Debug target]', { fullSlug, targetSlug, targetLast })
    console.log('[findPost Debug allPages]', props.allPages.map(p => ({ title: p.title, slug: p.slug, id: p.id, type: p.type })))

    return (
      // 1. 完整 slug 精准匹配
      props.allPages.find(p => p && !p.type?.includes('Menu') && normalize(p.slug) === targetSlug) ||
      // 2. 短别名匹配 (例如访问 /sss 命中 /article/sss，或反之)
      props.allPages.find(p => p && !p.type?.includes('Menu') && (
        normalize(p.slug) === `article/${targetSlug}` ||
        normalize(p.slug).endsWith(`/${targetLast}`) ||
        normalize(p.slug) === targetLast
      )) ||
      // 3. UUID 匹配
      props.allPages.find(p => p?.id === fullSlug || normalize(p?.id) === targetSlug) ||
      null
    )
  }
...
async function convertNotionToSiteData(
  ...
  // 注入管理后台覆盖配置，保证前台 SSG / ISR 静态生成与客户端渲染时实时生效
  let adminOverrides = {}
  try {
    adminOverrides = require('@/lib/adminConfigOverrides.json')
  } catch (e) {}
  // 尝试从 /tmp/adminConfigOverrides.json 读取（Serverless 临时持久化兜底）
  try {
    const fs = require('fs')
    const tmpConfig = '/tmp/adminConfigOverrides.json'
    if (fs.existsSync(tmpConfig)) {
      adminOverrides = { ...adminOverrides, ...JSON.parse(fs.readFileSync(tmpConfig, 'utf-8')) }
    }
  } catch (e) {}
  if (typeof globalThis !== 'undefined' && globalThis.__adminConfigOverrides) {
    adminOverrides = { ...adminOverrides, ...globalThis.__adminConfigOverrides }
  }

  // 智能过滤占位符：严防模板硬编码劫持真实 Notion 页面，但保留用户在后台主动配置的有效自定义值
  const SYSTEM_PLACEHOLDERS = ['NotionNext BLOG', 'Notion Repo BLOG', 'Notion Blog', '基于 Notion 的静态博客', 'NotionNext', 'Notion Next 知识库']
  const sanitizedAdminOverrides = { ...adminOverrides }

  // NOTION_PAGE_ID 关系到核心数据源，严禁被前端后台配置覆盖破坏
  delete sanitizedAdminOverrides['NOTION_PAGE_ID']

  // 若 TITLE/DESCRIPTION/AUTHOR/BIO 为空或占位符，予以过滤，确保回退至用户真实 Notion 页面属性
  ;['TITLE', 'DESCRIPTION', 'AUTHOR', 'BIO'].forEach(k => {
    const val = sanitizedAdminOverrides[k]
    if (!val || SYSTEM_PLACEHOLDERS.includes(val)) {
      delete sanitizedAdminOverrides[k]
    }
  })

  // 配置合并体系：云端持久化配置表 (Cloud Notion Config) 作为基准覆盖，后台即时覆写 (sanitizedAdminOverrides) 具有最高决策优先级
  const rawNotionConfig = (await getConfigMapFromConfigPage(collectionData)) || {}
  const NOTION_CONFIG = {
    ...rawNotionConfig,
    ...sanitizedAdminOverrides
  }
  if (process.env.NODE_ENV === 'development') {
    console.log(
  ...
  const sortBy = siteConfig('POSTS_SORT_BY', 'date', NOTION_CONFIG)
  if (sortBy === 'date' || sortBy === 'date-desc' || !sortBy) {
    allPages.sort((a, b) => {
      // 一级排序：按发布日期倒序
      const dateA = a?.publishDate ?? 0
      const dateB = b?.publishDate ?? 0
      const dayA = a?.publishDay || (dateA ? new Date(dateA).toISOString().slice(0, 10) : '')
      const dayB = b?.publishDay || (dateB ? new Date(dateB).toISOString().slice(0, 10) : '')
      ...
      // 同日发布（或日期未设具体时间时）的二级智能排序：
      // 1. 若在 Notion 中手动指定了具体时分 (start_time)，按具体时间倒序
      const timeA = a?.date?.start_time || ''
      const timeB = b?.date?.start_time || ''
      ...
      // 2. 严格按 Notion 底层精确创建时间 createdTime（或编辑时间）倒序，保证最新创建的文章永远排在最前面！
      const createdA = a?.createdTime || a?.lastEditedTime || 0
      const createdB = b?.createdTime || b?.lastEditedTime || 0
      return createdB - createdA
    })
  } else if (sortBy === 'date-asc') {
    allPages.sort((a, b) => (a?.publishDate ?? 0) - (b?.publishDate ?? 0))
  } else if (sortBy === 'views' || sortBy === 'views-desc') {
    allPages.sort((a, b) => (b?.page_views ?? b?.views ?? 0) - (a?.page_views ?? a?.views ?? 0))
  }
  ...
  // 触发异步非阻塞自动补齐缺失的 fans_code 与 vip_level，完全不阻塞当前渲染返回
  triggerAsyncPropertyBackfill(allPages)
...
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG }) {
  const defaultTitle = NOTION_CONFIG?.TITLE || 'Notion Repo BLOG'
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION || '这是一个由Notion Repo生成的站点'
  ...
  // 站点标题优先级：
  // 1. 若站长在后台主动配置了有效的自定义 TITLE（非默认占位符），以站长后台配置为最高优先级
  // 2. 其次读取 Notion 原生数据库名称 collection?.name
  // 3. 最后回退至兜底默认标题
  let title = defaultTitle
  if (NOTION_CONFIG?.TITLE && !SYSTEM_PLACEHOLDERS.includes(NOTION_CONFIG.TITLE)) {
    title = NOTION_CONFIG.TITLE
  } else if (collection?.name?.[0]?.[0]) {
    title = collection.name[0][0]
  }

  // 站点描述优先级：
  // 1. 若站长在后台主动配置了有效的自定义 DESCRIPTION（非默认占位符），以站长后台配置为最高优先级
  // 2. 其次读取 Notion 原生数据库描述
  // 3. 最后回退至兜底默认描述
  let description = defaultDescription
  if (NOTION_CONFIG?.DESCRIPTION && !SYSTEM_PLACEHOLDERS.includes(NOTION_CONFIG.DESCRIPTION)) {
    description = NOTION_CONFIG.DESCRIPTION
  } else if (collection?.description?.[0]?.[0]) {
...
/**
 * 静默异步补齐 Notion 文章缺失的 fans_code 或 vip_level 属性
 * 异步非阻塞，在后台执行，完全不阻塞当前 API 与页面的响应返回
 */
function triggerAsyncPropertyBackfill(allPages) {
  if (!Array.isArray(allPages) || allPages.length === 0) return
  // 只在具备 Notion API Token 环境下触发
  if (!process.env.NOTION_API_TOKEN && !process.env.NOTION_ACCESS_TOKEN && !process.env.NOTION_TOKEN) return

  // 筛选出勾选了 fans 但无 fans_code，或勾选了 vip 的文章
  const candidatePosts = allPages.filter(post => {
    if (!post?.id) return false
    const needFans = post.fans === true && (!post.fans_code || String(post.fans_code).trim() === '')
    const needVip = post.vip === true
    return needFans || needVip
  })
  ...
  setTimeout(async () => {
    try {
      const { syncSingleNotionArticle } = await import('@/lib/member/notion')
      for (const post of candidatePosts) {
        await syncSingleNotionArticle(post.id)
      }
    } catch (e) {
      console.warn('[SiteDataApi] 异步自动补齐 Notion 属性捕获异常:', e.message)
    }
  }, 100)
}
