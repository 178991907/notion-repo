...
const ExampleRecentComments = (props) => {
  ...
  return <>
        ...
        {!onLoading && comments && comments.length > 0 && comments.map((comment) => <div key={comment.objectId} className='pb-2'>
            <div className='dark:text-gray-300 text-gray-600 text-xs waline-recent-content wl-content' dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(comment.comment) }} />
