...
Notion Repo支持多种评论插件，通常只需要准备好对应的评论功能，在Notion Repo的环境变量中添加评论插件的信息即可启用。

目前支持7种评论插件，其中部署最快速的是 **Cusdis** 和 **Utterance**这两个插件，他们几乎不需要另外的设置，直接配置到Notion Repo即可。
...
如果只是给博客、文档站或个人网站增加评论，优先选择现有评论插件。Twikoo、Waline、Artalk、Giscus、Cusdis 等方案已经覆盖了评论存储、审核、通知、反垃圾和迁移等常见需求，不需要把评论系统重新写进 Notion Repo。
...
不建议一开始把支付、订单、积分、复杂 RBAC 全塞进 Notion Repo。Notion 适合当“站长可视化后台”和低频权限源，不适合当高并发交易数据库。评论如果流量较大，仍建议使用 Waline、Artalk、Twikoo 或自建数据库，并保留登录身份绑定。
