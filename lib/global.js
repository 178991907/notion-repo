...
/**
 * 安全包装 Clerk useUser hook
 * 在没有 ClerkProvider 或 Clerk 未正确初始化时返回安全默认值
 * 注意：此 hook 内部使用 require 动态加载 Clerk，避免静态 import 导致 SDK 初始化崩溃
 */
function useClerkUserSafe() {
  const defaultResult = { isLoaded: true, isSignedIn: false, user: null }
  const enableClerk = Boolean(
    process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY &&
    process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY.startsWith('pk_')
  )
  // enableClerk 是编译时常量（Next.js 内联 NEXT_PUBLIC_ 环境变量），条件分支会被死代码消除
  if (!enableClerk) return defaultResult
  try {
    // 通过变量重命名避免 ESLint react-hooks/rules-of-hooks 误报
    // （enableClerk 在构建时是常量，不存在真正的条件调用）
    const clerkModule = require('@clerk/nextjs')
    const clerkHook = clerkModule.useUser
    const result = clerkHook()
...
export function GlobalContextProvider(props) {
  ...
  // 登录验证相关 (使用安全包装，防止 Clerk Context 未就绪时崩溃)
  /* eslint-disable-next-line react-hooks/rules-of-hooks */
  const clerkData = useClerkUserSafe()
  const isLoaded = clerkData.isLoaded ?? true
  const isSignedIn = clerkData.isSignedIn ?? false
  const user = clerkData.user ?? null
  ...
  useEffect(() => {
    ...
    // 处理多语言自动重定向（必须明确开启时才执行）
    const redirectLang = siteConfig('REDIRECT_LANG')
    if (
      redirectLang === true ||
      redirectLang === 'true' ||
      (NOTION_CONFIG?.REDIRECT_LANG && JSON.parse(NOTION_CONFIG?.REDIRECT_LANG) === true)
    ) {
