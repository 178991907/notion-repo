...
## 创建 Notion 连接密钥（Integration Token）

很多新手不理解什么是“Integration”：
你可以把它理解为**“给你的博客系统配一把专属门禁钥匙”**。
为了保障你的 Notion 账户绝对安全，Notion 不允许任何外部系统直接使用你的账号密码登录；而是通过一个专属的“内部连接密钥”（以 `secret_` 开头），让博客只能在你授权的页面里安全读写评论与会员数据。

> 💡 **核心优势**：只要配置了这个连接密钥，暗号自动核销、VIP 自动发放、免登录匿名评论、管理后台全自动化都会自动打通，完全无需任何第三方数据库或付费服务！

### 1. 新建连接并获取密钥

1. 打开 [Notion 官方集成中心](https://www.notion.so/profile/integrations)（建议右键新标签页打开）。
2. 点击 **`New integration`**（或 **`新建集成`**）。
3. **名称**：随意填写一个你认识的名字，例如 `Notion-Repo`（这个名字在后面授权时会显示）。
4. **关联工作区**：选择你的博客所在的 Notion 工作区。
5. **类型**：默认选择 `内部集成`（Internal integration）。
6. 点击提交保存，复制生成的 **Internal Integration Secret**（内部集成密钥，以 `secret_` 开头）。
7. 将这个密钥配置到你部署平台的环境变量 **`NOTION_ACCESS_TOKEN`** 中（注：系统同时向下兼容 `NOTION_TOKEN`）。

### 2. 在 Notion 博客页面添加连接授权（最关键的一步！）

> ⚠️ **新手注意**：拿到密钥后，**必须在 Notion 页面里为它开通“门禁权限”**，否则系统无法读取或写入数据。

具体操作步骤（只需 10 秒）：

1. 打开你的 **Notion 博客主页面**（即 `NOTION_PAGE_ID` 对应的页面）。
2. 点击页面右上角的三个点 **「···」**。
3. 在下拉菜单倒数几行找到 **「品 集成」**（英文版为 **Connect to**）。
4. 点击 **「+ 添加连接」**，在弹出的列表中找到你在第 1 步填写的连接名称（例如 `Notion-Repo`）。
5. 点击蓝色的 **「添加到页面」** 确认授权！

![Notion 页面添加连接授权指引](/images/notion-connect-guide.png)

系统具备自动穿透与建库引擎，只要主页面完成了添加连接，系统就会自动在主页面下挂载数据存储中心并自动创建评论库与会员库，无需手动建表！
