...
本指南用于把外部主题（例如 Astro/Vite 主题）迁移到 Notion Repo 的 Next.js + Notion 数据架构中。

## 1）迁移目标

- 尽量保留原主题的视觉语言（布局、间距、卡片、动效）。
- 遵循 Notion Repo 的数据流与功能约定。
...
你在本地或自建 fork 中，可随时将自定义主题放在 **`themes/<主题目录名>/`** 下使用，**不强制**提交到 Notion Repo 上游。

若希望主题被 **合并进 Notion Repo 官方主仓库**，除完整主题代码外，还需补齐「主题切换」浮窗中的 **预览图** 与 **文案介绍**（站点启用 `THEME_SWITCH` / `NEXT_PUBLIC_THEME_SWITCH` 时展示）。
...
- **承认投入**：完整主题涉及视觉、交互、多端适配与长期兼容，开发与维护成本高；向 Notion Repo **公开主仓库**贡献可再分发主题，是对社区的重要支持。
