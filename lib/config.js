...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
      ...
      return convertVal(
        getValue(
          (typeof window === 'undefined' && typeof globalThis !== 'undefined' && globalThis.__adminConfigOverrides?.[key] !== undefined)
            ...
            : (getGlobalSnapshot()?.NOTION_CONFIG?.[key] !== undefined
                ? getGlobalSnapshot().NOTION_CONFIG[key]
                : adminOverrides?.[key]),
  ...
  // 2. 第二优先级：服务端进程最新保存的内存覆盖（若存在）
  if (!hasVal(val) && typeof globalThis !== 'undefined' && globalThis.__adminConfigOverrides?.[key] !== undefined) {
    val = globalThis.__adminConfigOverrides[key]
  }

  // 3. 第三优先级：云端配置中心 Notion 数据库拉取的动态持久化配置 (NOTION_CONFIG)
  if (!hasVal(val) && globalSnapshot?.NOTION_CONFIG?.[key] !== undefined) {
    val = globalSnapshot.NOTION_CONFIG[key]
  }

  // 4. 第四优先级：本地纯离线开发降级文件（仅在云端与内存均无该配置时生效，绝不覆盖动态数据）
  if (!hasVal(val) && adminOverrides?.[key] !== undefined) {
    val = adminOverrides[key]
  }
  ...
  if (isThemeKey) {
    // 主题专有配置：若云端及后台均未设置，依次从 extendConfig 与 THEME_CONFIG 默认值回退
    if (!hasVal(val) && extendConfig && extendConfig[key] !== undefined) {
      val = extendConfig[key]
    }
    if (!hasVal(val) && globalSnapshot?.THEME_CONFIG?.[key] !== undefined) {
      val = globalSnapshot.THEME_CONFIG[key]
    }
  } else {
    // 通用非主题配置：局部传参 extendConfig 回退
    if (!hasVal(val) && extendConfig && extendConfig[key] !== undefined) {
      val = extendConfig[key]
    }
  }
  ...
  if (!hasVal(val)) {
    // 这里针对部分基础站点信息做兼容处理与回退
    switch (key) {
      case 'HOME_BANNER_IMAGE':
        val = siteInfo?.pageCover || val // 封面图优先取Notion的封面
        break
      case 'AVATAR':
        val = siteInfo?.icon || val // 头像优先取Notion的头像
        break
      case 'TITLE':
        val = siteInfo?.title || val // 站点名称在后台未指定时回退至用户真实Notion页面标题
        break
      case 'DESCRIPTION':
        val = siteInfo?.description || val // 站点描述在后台未指定时回退至用户真实Notion页面描述
        break
