 ...
 * 粉丝专区免登录验证码工具模块
 * 提供通用暗号与单篇独立暗号双模式验证
 * 验证成功后在客户端本地存储解锁凭证（默认 1 天有效期），免去当天重复输入
 */
...
export function verifyFansPasscode(inputPasscode, articleFansCode, defaultPasscode = '888888') {
  ...
  // 解析文章配置的独立验证码列表（支持数组、逗号、顿号、分号或空格分隔的多暗号）
  let allowedCodes = []
  if (Array.isArray(articleFansCode)) {
    allowedCodes = articleFansCode
      .flatMap(c => (typeof c === 'string' ? c.split(/[,，;\s/、]+/) : String(c)))
      .map(c => String(c).trim().toLowerCase())
      .filter(Boolean)
  } else if (articleFansCode) {
    allowedCodes = String(articleFansCode)
      .split(/[,，;\s/、]+/)
      .map(c => c.trim().toLowerCase())
      .filter(Boolean)
  }

  // 1. 优先匹配该文章设置的独立专属验证码（支持多个专属码标签）
  if (allowedCodes.length > 0 && allowedCodes.includes(cleanInput)) {
    return { valid: true, isGlobal: false }
  }

  // 2. 全站默认通用暗号（公众号主暗号，双轨兜底放行）
  // 无论文章是否生成了专属独立随机码，用户输入公众号通用主暗号均可顺畅解锁
  ...
  if (allowedCodes.length > 0) {
    return { valid: false, isGlobal: false, message: '验证码或暗号不正确，请核对后再试' }
  }
